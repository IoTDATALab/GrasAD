...
class AsynClientsAvgAggregator(ClientsAvgAggregator):
    ...
    def aggregate(self, agg_info):
        ...
        if self.cfg.usrsele.use + self.cfg.usrsele_compa_schandagg.use + self.cfg.usrsele_compa_freqandage.use \
                + self.cfg.usrsele_compa_hfl.use >= 1:
            ...
            dists = [models_plus_dist[i][-1] for i in range(len(models_plus_dist))]  # 分离出训练集分布
        else:
        ...
        if self.cfg.usrsele.use == True:
            ...
            # print('AAAAAAAAAAAAAAAAAAAAAAAAAA',weight_list)
    ...
    def _weight_func_ours(self, models, staleness):
        ...
        if self.global_num == 0:
            ...
            for i, model in enumerate(models):
                ...
                out_sim.append(-sim) if sim < 0 else out_sim.append(sim)
            ...
            for i in range(model_num):
                weight.append(0.3 * out_sim[i] + 0.7 * in_sim[i])
                # weight.append(0.3 * out_sim[i] + 0.7 * (score / (model_num - 1)))
        weight_list = [weight[i] / sum(weight) for i in range(len(weight))]
        return weight_list
    ...
    def _client_select_comp_hfl(self, models, staleness, client_id, distr,):
       ...
       # 需要筛选的设备数量/
        ...
        list2 = [index for index, value in list1]
        list3 = [client_id[index] for index in list2]
        return list3
