...
plt.xticks([0.4, 50, 79],['$\epsilon=0$','5.67','7.51'])
...
# plt.plot(data11[0::2],'--')
df = pd.read_table(r'results/celeba_median.log', sep=',')
...
plt.plot(data11[0::2])
plt.xticks([0, 20, 37], ['0', '18.62', '23.5'])
...
plt.subplot(grid[0,3])
plt.title('D (Reddit)')
df = pd.read_table(r'results/reddit_median.log', sep=',')
data11 = [float(df.values[i][3][12:]) for i in range(1, len(df)-1)]
plt.plot(data11,'--')
df = pd.read_table(r'results/reddit_adaclip2.log', sep=',')
data11 = [float(df.values[i][4][12:]) for i in range(1, len(df)-1)]
...
plt.plot(data11)
plt.grid(axis='y', linestyle='--')
plt.xticks([0.4, 10, 17], ['0', '81.75', '151.9'])
